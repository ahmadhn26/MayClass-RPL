...
<div class="tentor-form-card">
    ...
    <div class="tentor-form-grid">
        <div class="tentor-form-group">
            ...
            <input type="text" name="headline" value="{{ old('headline', optional($tentorProfile)->headline) }}"
        ...
        <div class="tentor-form-group">
            ...
            <input type="text" name="specializations"
                value="{{ old('specializations', optional($tentorProfile)->specializations) }}" required
                placeholder="Contoh: Matematika SMA, Fisika SMP">
        ...
        <div class="tentor-form-group">
            ...
            <input type="number" name="experience_years" min="0" max="60"
                value="{{ old('experience_years', optional($tentorProfile)->experience_years) }}">
            @error('experience_years')<p class="helper-text" style="color:#dc2626;">{{ $message }}</p>@enderror
        </div>
        <div class="tentor-form-group">
            <label>Pendidikan Terakhir</label>
            <input type="text" name="education" value="{{ old('education', optional($tentorProfile)->education) }}"
    ...
    <div class="tentor-form-group" style="margin-top:24px;">
        ...
        <textarea name="bio"
            placeholder="Ceritakan metode mengajar, pengalaman lomba, atau pendekatan belajar.">{{ old('bio', optional($tentorProfile)->bio) }}</textarea>
                    ...
                    <div class="subject-checkboxes">
                        @foreach($subjectsByLevel[$level] as $subject)
                            <label class="checkbox-label">
                                <input type="checkbox" name="subjects[]" value="{{ $subject->id }}" {{ in_array($subject->id, old('subjects', $tentor?->subjects->pluck('id')->toArray() ?? [])) ? 'checked' : '' }}>
                                {{ $subject->name }}
                            </label>
                        @endforeach
                    </div>
