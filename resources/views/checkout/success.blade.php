...
<body>
    ...
    @php
        ...
        $totalFormatted = number_format((int) $order->subtotal, 0, ',', '.');
    @endphp
    ...
    <main class="success-card">
        ...
        <p class="description">
            @if ($isPending)
                Terima kasih! Bukti pembayaran Anda telah kami terima. Admin akan memverifikasi data Anda maksimal
                <strong>1x24 jam</strong>.
            @else
                Selamat! Paket <strong>{{ $pkgTitle }}</strong> sudah aktif. Anda sekarang memiliki akses penuh ke semua
                materi.
            @endif
        </p>
        ...
        @if ($isPending)
            <div
                style="background: var(--primary-light); border: 1px solid rgba(15, 118, 110, 0.2); border-radius: 12px; padding: 16px; margin-bottom: 24px; text-align: center;">
        ...
        <a href="{{ $isPending ? route('home') : route('student.dashboard') }}" class="btn-primary">
            {{ $isPending ? 'Kembali ke Beranda' : 'Masuk ke Dashboard' }}
        </a>
