...
@push('styles')
    <style>
        ...
        .quiz-card { background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: var(--radius-lg); padding: 20px; display: flex; gap: 20px; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative; overflow: hidden; }
        .quiz-card:hover { transform: translateY(-4px); box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); border-color: #cbd5e1; }
        .quiz-thumbnail { width: 100px; height: 100px; flex-shrink: 0; border-radius: 12px; object-fit: cover; background: #f1f5f9; }
        ...
        .tags-row { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 12px; font-size: 0.75rem; }
        ...
        .btn-outline:hover { border-color: var(--accent-purple); color: var(--accent-purple); background: #f5f3ff; }
        ...
        .btn-close { background: none; border: none; color: #64748b; cursor: pointer; padding: 8px; transition: color 0.2s; }
        .btn-close:hover { color: #ef4444; }

        .form-group { margin-bottom: 20px; }
        .form-label { display: block; font-size: 0.9rem; font-weight: 600; color: #334155; margin-bottom: 8px; }
        .form-control { width: 100%; padding: 12px 16px; border: 1px solid #cbd5e1; border-radius: 10px; font-size: 0.95rem; background: #f8fafc; transition: border-color 0.2s; }
        .form-control:focus { outline: none; border-color: var(--primary-color); background: white; box-shadow: 0 0 0 3px rgba(63, 166, 126, 0.1); }
        ...
        .btn-submit { width: 100%; padding: 14px; background: var(--primary-color); color: white; border: none; border-radius: 12px; font-size: 1rem; font-weight: 700; cursor: pointer; transition: background 0.2s; margin-top: 16px; }
        .btn-submit:hover { background: var(--primary-dark); }

        @media (max-width: 640px) {
            .quiz-card { flex-direction: column; }
            .quiz-thumbnail { width: 100%; height: 160px; }
...
@section('content')
                ...
                <article class="quiz-card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-actions">
                            <a href="{{ route('tutor.quizzes.edit', $quiz) }}" class="action-btn btn-secondary">Edit</a>
                            @if ($quiz->link)
                                <a href="{{ $quiz->link }}" class="action-btn btn-outline" target="_blank" rel="noopener" title="Buka link quiz">
                                   Buka Quiz
    ...
    {{-- --- MODERN MODAL STRUCTURE FOR QUIZ --- --}}
    <div id="createModal" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title">Buat Quiz Baru</h2>
            ...
            <form action="{{ route('tutor.quizzes.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" class="form-control" placeholder="Contoh: Evaluasi Trigonometri" required>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Link Quiz (Google Form/CBT)</label>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="duration_label" class="form-control" placeholder="e.g., 90 Menit" required>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea name="summary" rows="2" class="form-control" placeholder="Tentang quiz ini..." required></textarea>
                </div>

                {{-- Tingkat Kesulitan Dinamis --}}
                <div class="form-group">
                    <label class="form-label">Level / Kesulitan (Opsional)</label>
                    <ul id="levelsList" class="dynamic-list">
                        <li class="dynamic-item">
                            <input type="text" name="levels[]" class="form-control" placeholder="e.g., Mudah">
                        </li>
                    </ul>
                    <button type="button" class="btn-add-item" onclick="addLevel()">+ Tambah Level</button>
                </div>

                {{-- Poin Penting Dinamis --}}
                <div class="form-group">
                    <label class="form-label">Poin Penting / Takeaways</label>
                    <ul id="takeawaysList" class="dynamic-list">
                        <li class="dynamic-item">
                            <input type="text" name="takeaways[]" class="form-control" placeholder="Apa yang didapat siswa?">
                        </li>
                    </ul>
                    <button type="button" class="btn-add-item" onclick="addTakeaway()">+ Tambah Poin</button>
                </div>
    ...
    <script>
        ...
        function fetchSubjects(packageId) {
            ...
            fetch(`/tutor/quizzes/packages/${packageId}/subjects`)
        ...
        function addLevel() {
            ...
            li.innerHTML = `
                <input type="text" name="levels[]" class="form-control" placeholder="Level lainnya...">
        ...
        function addTakeaway() {
            ...
            li.className = 'dynamic-item';
            li.innerHTML = `
                <input type="text" name="takeaways[]" class="form-control" placeholder="Poin penting lainnya...">
                <button type="button" class="btn-remove-item" onclick="this.parentElement.remove()">×</button>
            `;
            list.appendChild(li);
        }
